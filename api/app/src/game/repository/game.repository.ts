...
export class GameRepository extends Repository<Game> {
    constructor(private readonly datasource: DataSource) {
        super(Game, datasource.createEntityManager(), datasource.createQueryRunner());
    }

    // 비동기 처리를 위한 미래에 무언가 데이터가 올것이다! Promise
    async getById(id: number): Promise<Game>{
        return await this.datasource.createEntityManager().findOneById(Game, id)
    }
}
...
// super란 parent에 접근 아래와 같은 인자를 넣어서 생성하는 부모 생성자 로 여기서는 Gametype인 reposiroty
